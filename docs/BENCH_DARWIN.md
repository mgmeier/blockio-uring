...
|---|---|---:|---:|
| Low-level | Cache (page cache on) | ~272,000 | 1,074 B |
| Low-level | NoCache (`F_NOCACHE`) | ~304,000 | 1,074 B |
| High-level | Cache (page cache on) | ~262,000 | 142 B |
| High-level | NoCache (`F_NOCACHE`) | ~262,000 | 140 B |
...
**Low-level outperforms high-level (~304K vs ~262K NoCache).** The low-level
...
latency between individual ops.  The 15% gap is the cost of those safety and
...
are essentially identical (~262K); for the low-level API NoCache is actually
slightly *faster* (~304K vs ~272K, with ~10–15% run-to-run variance).  Two
...
`kern.aiomax` limits.  The absolute IOPS ceiling — in the 260–305K range for
