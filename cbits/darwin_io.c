...
struct darwin_ring {
    ...
    /* Pending SQEs: prepared between get_sqe and submit.
...
darwin_ring *darwin_ring_create(unsigned sq_size, unsigned cq_size) {
    (void)cq_size;  /* GCD manages its own thread pool */
...
darwin_sqe *darwin_get_sqe(darwin_ring *ring) {
    if (ring->pending_count >= ring->pending_cap) return NULL;
    return &ring->pending[ring->pending_count++];
}
